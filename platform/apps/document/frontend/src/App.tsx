...
export const App: React.FC = () => {
  ...
  useEffect(()=>{if(!focusPreview)return;const exit=(event:KeyboardEvent)=>{if(event.key==='Escape')setFocusPreview(false);};window.addEventListener('keydown',exit);return()=>window.removeEventListener('keydown',exit);},[focusPreview]);
  useEffect(()=>{const sync=()=>{if(!document.fullscreenElement)setFocusPreview(false);};document.addEventListener('fullscreenchange',sync);return()=>document.removeEventListener('fullscreenchange',sync);},[]);
  ...
  const downloadSelected=()=>{const objectUrl=selected.sourceFile?URL.createObjectURL(selected.sourceFile):'';const link=document.createElement('a');link.href=objectUrl||`/api/documents/content?path=${encodeURIComponent(selected.id)}`;link.download=selected.name;document.body.appendChild(link);link.click();link.remove();if(objectUrl)setTimeout(()=>URL.revokeObjectURL(objectUrl),1000);};
  const enterFullscreen=()=>{setFocusPreview(true);const request=document.documentElement.requestFullscreen;if(!document.fullscreenElement&&typeof request==='function')request.call(document.documentElement).catch(()=>{});};
  const leaveFullscreen=()=>{setFocusPreview(false);if(document.fullscreenElement)document.exitFullscreen().catch(()=>{});};
