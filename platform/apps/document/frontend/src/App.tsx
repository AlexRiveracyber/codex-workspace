...
type DocumentItem = { id: string; name: string; path: string; extension: string; size: number; modified: string; favorite?: boolean };
...
export const App: React.FC = () => {
  ...
  const [loading,setLoading]=useState(true); const [error,setError]=useState(''); const [previewText,setPreviewText]=useState('');
  ...
  useEffect(()=>{load();},[]);
  useEffect(()=>{setPreviewText('');if(text.has(selected.extension))fetch(`/api/documents/content?path=${encodeURIComponent(selected.id)}`).then(r=>r.ok?r.text():'').then(setPreviewText).catch(()=>{});},[selected]);
  ...
  const Icon=icon(selected.extension); const url=`/api/documents/content?path=${encodeURIComponent(selected.id)}`;
  return <div className="app">
    ...
    <main>
      <aside><div className="library"><HardDrive/><div><b>{library.name}</b><small>{library.documents.length} 个文档</small></div></div><label>资料库</label><button className={folder==='全部文档'?'active':''} onClick={()=>setFolder('全部文档')}><HardDrive/>全部文档<em>{library.documents.length}</em></button><button><Star/>我的收藏<em>{library.documents.filter(d=>d.favorite).length}</em></button><label>文件夹</label>{folders.slice(1).map(f=><button key={f} className={folder===f?'active':''} onClick={()=>setFolder(f)}><Folder/>{f}</button>)}<div className="mount"><Folder/><b>只读挂载</b><p>{library.root}</p><small>修改 DOCUMENT_LIBRARY_PATH 可切换本地资料目录</small></div></aside>
