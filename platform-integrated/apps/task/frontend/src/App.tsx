...
export const App: React.FC = () => {
  ...
  const handleClearForm = () => {
    ...
    currentMenu.fields.forEach((f) => {
      if (f.key === 'req_date') emptyData[f.key] = getTodayDate();
      else if (f.key === 'req_seq_id') emptyData[f.key] = generateReqSeqId(currentMenu.id);
      else if (f.type === 'array') emptyData[f.key] = [];
  ...
  const compactPayloadValue = (value: any): any => {
    if (value === undefined || value === null) return undefined;
    if (typeof value === 'string') return value.trim() === '' ? undefined : value;
    if (Array.isArray(value)) {
      const compacted = value.map(compactPayloadValue).filter((item) => item !== undefined);
      return compacted.length > 0 ? compacted : undefined;
    }
    if (typeof value === 'object') {
      const compacted = Object.fromEntries(
        Object.entries(value)
          .map(([key, childValue]) => [key, compactPayloadValue(childValue)] as const)
  ...
  // Serialize only populated payload fields; objects & arrays are serialized to JSON strings.
  const serializePayload = useMemo(() => {
    const payload: Record<string, any> = {};
    currentMenu.fields.forEach((f) => {
      const val = compactPayloadValue(formData[f.key]);
      if (val === undefined) return;
      payload[f.key] = f.type === 'object' || f.type === 'array' ? JSON.stringify(val) : String(val);
    });
    return payload;
  ...
  const handleApiCall = async (endpoint: string) => {
    ...
    const requestPayload: Record<string, any> = {
      ...serializePayload,
      ...(activeConfig?.id !== undefined && activeConfig?.id !== null ? { config_id: activeConfig.id } : {}),
    };
    ...
    <div className="task-console min-h-screen bg-[#f8fafc] text-slate-800 font-sans flex flex-col selection:bg-blue-500 selection:text-white">
      ...
      <div className="task-workspace flex-1 flex w-full p-3 md:p-4 gap-4 overflow-hidden">
        <div className="task-nav-stack">
          ...
          <aside
            className={`task-endpoint-panel ${sidebarCollapsed ? 'is-collapsed' : 'is-open'}`}
            aria-label="接口目录"
        ...
        <main className="task-main flex-1 min-w-0 overflow-y-auto space-y-4 pr-1 pb-24">
          ...
          <div
            className={`task-action-dock fixed bottom-3 right-4 z-40 ${
